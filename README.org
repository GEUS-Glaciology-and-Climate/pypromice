...
geometry    = POINT(lon, lat)  # deg.frac E (W is negative), deg.frac N
...
infile = "./data/L0/EGP/EGP_2019_raw.txt.hdr"
#+END_SRC
...
def time_shift(da):
    ...
    if ds.attrs['format'] == 'STM':
        return da['time'] + pd.to_timedelta("-1 hour")
    if ds.attrs['format'] == 'TX':
        diff = da['time'].diff(dim='time')
        diffarr = diff.values.astype('timedelta64[h]').astype(np.int)
        ...
        # da['time'] = (da['time'] + pd.to_timedelta("-24 hours"))\
        #     .where((diffarr == 24) & ((da['time'].dt.dayofyear > 300) | (da['time'].dt.dayofyear < 100)), other=da['time'])
        # da['time'] = da['time'].where((diffarr == 1) | (diffarr == 24)) # nan if shift not 1 or 24 hours
        return da['time']
    
if ds.attrs['format'] != 'raw':
    ds['time_orig'] = ds['time']
    ds['time'] = time_shift(ds['time'].copy(deep=True)).values
...
# # # %matplotlib qt
# ds[['time','time_orig']].to_dataframe().iloc[2130:].head(30)

# ds = ds.isel({'time':~ds.time.isnull()}) # drop invalid times that got shifted
...
ds.to_dataframe().dropna(how='all').to_csv(outpathfile+".csv", float_format="%.4f")
...
# # L0 -> L1
# for f in $(ls ./data/L0/${s}/*raw*.hdr | grep -v Slim | grep -v transm); do
#   echo $f
#   ./L0_to_L1.py ${f}
# done
# parallel --bar "./L0_to_L1.py {}" ::: $(ls ./data/L0/${s}/*.hdr)
# ./L0_to_L1.py ./data/L0/EGP/EGP_2016_raw.txt.hdr
# ./L0_to_L1.py ./data/L0/EGP/EGP_2017_raw_2.txt.hdr
# ./L0_to_L1.py ./data/L0/EGP/EGP_2018_raw.txt.hdr
# ./L0_to_L1.py ./data/L0/EGP/EGP_2018_raw_2.txt.hdr
# ./L0_to_L1.py ./data/L0/EGP/EGP_2019_raw.txt.hdr
# ./L0_to_L1.py ./data/L0/EGP/EGP_2019_raw1.txt.hdr
##
./L0_to_L1.py ./data/L0/EGP/EGP_2017_raw_SlimTableMem.txt.hdr
##
# ./L0_to_L1.py ./data/L0/EGP/EGP_2019_raw_transmitted.txt.hdr
...
# L1 -> L1A
# for f in $(ls ./data/L1/${s}/); do
#   echo $f
#   ./L1_to_L1A.py ./data/L1/${s}/${f}
# done
...
def read_L0(filename):
    ...
    for req in ["columns", "station_id", "format", "geometry"]: assert(req in hdr.keys())

    xyz = np.array(re.findall("[-+]?[\d]*[.][\d]+", hdr['geometry'])).astype(np.float)
    hdr['longitude'] = xyz[0]
    hdr['latitude'] = xyz[1]
    hdr['altitude'] = xyz[2] if len(xyz) == 3 else np.nan
...
df = pd.read_csv("./data/L3/EGP/EGP-STM.csv", index_col=0, parse_dates=True)
# df = pd.read_csv("./data/L3/EGP/EGP-TX.csv", index_col=0, parse_dates=True)
...
dd = err.describe().T.drop(columns="count")
# dd = err.describe().T.drop(columns="count").replace(0,np.nan).dropna(how='all', axis='rows').replace(np.nan,0)

if __name__ == "__main__": 
    print(dd)
    dd.to_csv("tmp.csv", float_format='%.5f')

# diff_pct.plot()
# diff_pct.replace(0,np.nan).dropna(how='all', axis='columns').plot()
def plot_diff(df,idl,err,var):
    import matplotlib.pyplot as plt
    fig = plt.figure(1)
    fig.clf()
    ax1 = fig.add_subplot(211)
    err[var].plot(label='err', color='black', marker='.', ax=ax1)
    ax1.set_ylabel("Err")
...
var = 't_1'; plot_diff(df,idl,err,var)
...
:RESULTS:
#+begin_example
<ipython-input-26-bb2e0b59ad9d>:9: FutureWarning: The pandas.datetime class is deprecated and will be removed from pandas in a future version. Import from datetime module instead.
  return pd.datetime(int(y),int(mo),int(d),int(h),int(mi))
             mean     std      min      25%      50%     75%     max
albedo    0.00000 0.00029 -0.00050 -0.00020  0.00000 0.00020 0.00110
batt_v    0.00000 0.00000  0.00000  0.00000  0.00000 0.00000 0.00000
cc        0.00001 0.00278 -0.00500 -0.00230  0.00000 0.00230 0.00500
dlr      -0.00011 0.02866 -0.04990 -0.02460 -0.00080 0.02470 0.05000
dsr      -0.00058 0.02859 -0.05000 -0.02530  0.00000 0.02420 0.05000
dsr_cor  -0.00018 0.02433 -0.52200  0.00000  0.00000 0.00000 0.37740
...
gps_lat  -0.00000 0.00003 -0.00005 -0.00002 -0.00000 0.00002 0.00005
gps_lon   0.00000 0.00003 -0.00005 -0.00002  0.00000 0.00003 0.00005
p         0.00000 0.00000  0.00000  0.00000  0.00000 0.00000 0.00000
rh_cor   -0.00015 0.01376 -0.04990  0.00000  0.00000 0.00000 0.05000
t_1      -0.00000 0.00048 -0.00500  0.00000  0.00000 0.00000 0.00500
t_2      -0.00000 0.00043 -0.00500  0.00000  0.00000 0.00000 0.00500
t_i_1    -0.00000 0.00031 -0.00500  0.00000  0.00000 0.00000 0.00500
t_i_2     0.00000 0.00019 -0.00500  0.00000  0.00000 0.00000 0.00500
t_i_3     0.00000 0.00028 -0.00500  0.00000  0.00000 0.00000 0.00500
t_i_4     0.00000 0.00000  0.00000  0.00000  0.00000 0.00000 0.00000
t_i_5     0.00000 0.00000  0.00000  0.00000  0.00000 0.00000 0.00000
t_i_6     0.00000 0.00000  0.00000  0.00000  0.00000 0.00000 0.00000
t_i_7     0.00000 0.00000  0.00000  0.00000  0.00000 0.00000 0.00000
t_i_8     0.00000 0.00000  0.00000  0.00000  0.00000 0.00000 0.00000
t_surf    0.00004 0.00288 -0.00500 -0.00240  0.00010 0.00250 0.00500
tilt_x   -0.00001 0.00288 -0.00500 -0.00250  0.00000 0.00250 0.00500
tilt_y    0.00002 0.00289 -0.00500 -0.00250  0.00000 0.00250 0.00500
ulr       0.00035 0.02878 -0.05000 -0.02430  0.00080 0.02570 0.05000
usr       0.00041 0.02903 -0.04990 -0.02610  0.00000 0.02610 0.04990
usr_cor   0.00010 0.02089 -0.10720  0.00000  0.00000 0.00000 0.10450
wdir     -0.00005 0.00433 -0.05000  0.00000  0.00000 0.00000 0.05000
wspd     -0.00001 0.00264 -0.00500 -0.00200  0.00000 0.00200 0.00500
z_boom   -0.00000 0.00029 -0.00050 -0.00030  0.00000 0.00030 0.00050
z_pt          nan     nan      nan      nan      nan     nan     nan
z_stake       nan     nan      nan      nan      nan     nan     nan
#+end_example
|          |          mean |           std |        min |        25% |       50% |        75% |      max |
|----------+---------------+---------------+------------+------------+-----------+------------+----------|
| albedo   |   1.74326e-06 |   0.00028985  |  -0.0005   |  -0.0002   |   0       |   0.0002   |   0.0011 |
| batt_v   |   0           |   0           |   0        |   0        |   0       |   0        |   0      |
| cc       |   1.17763e-05 |   0.00278238  |  -0.005    |  -0.0023   |   0       |   0.0023   |   0.005  |
| dlr      |  -0.000111832 |   0.0286637   |  -0.0499   |  -0.0246   |  -0.0008  |   0.0247   |   0.05   |
| dsr      |  -0.000582256 |   0.0285922   |  -0.05     |  -0.0253   |   0       |   0.0242   |   0.05   |
| dsr_cor  |  -0.000179862 |   0.024329    |  -0.522    |   0        |   0       |   0        |   0.3774 |
| fan_dc   |   0           |   0           |   0        |   0        |   0       |   0        |   0      |
| gps_alt  |   0           |   0           |   0        |   0        |   0       |   0        |   0      |
| gps_hdop |   0           |   0           |   0        |   0        |   0       |   0        |   0      |
| gps_lat  |  -1.54557e-06 |   2.81153e-05 |  -4.98e-05 |  -2.48e-05 |  -3.7e-06 |   2.22e-05 |   5e-05  |
| gps_lon  |   8.46655e-07 |   2.85794e-05 |  -4.98e-05 |  -2.5e-05  |   2.3e-06 |   2.63e-05 |   5e-05  |
| p        |   0           |   0           |   0        |   0        |   0       |   0        |   0      |
| rh_cor   |  -0.000154917 |   0.0137561   |  -0.0499   |   0        |   0       |   0        |   0.05   |
| t_1      |  -1.40272e-06 |   0.000477023 |  -0.005    |   0        |   0       |   0        |   0.005  |
| t_2      |  -2.66517e-06 |   0.000429834 |  -0.005    |   0        |   0       |   0        |   0.005  |
| t_i_1    |  -1.82354e-06 |   0.000305894 |  -0.005    |   0        |   0       |   0        |   0.005  |
| t_i_2    |   1.40272e-06 |   0.000185011 |  -0.005    |   0        |   0       |   0        |   0.005  |
| t_i_3    |   7.01361e-07 |   0.000279539 |  -0.005    |   0        |   0       |   0        |   0.005  |
| t_i_4    |   0           |   0           |   0        |   0        |   0       |   0        |   0      |
| t_i_5    |   0           |   0           |   0        |   0        |   0       |   0        |   0      |
| t_i_6    |   0           |   0           |   0        |   0        |   0       |   0        |   0      |
| t_i_7    |   0           |   0           |   0        |   0        |   0       |   0        |   0      |
| t_i_8    |   0           |   0           |   0        |   0        |   0       |   0        |   0      |
| t_surf   |   3.81964e-05 |   0.00287877  |  -0.005    |  -0.0024   |   0.0001  |   0.0025   |   0.005  |
| tilt_x   |  -1.19562e-05 |   0.00287647  |  -0.005    |  -0.0025   |   0       |   0.0025   |   0.005  |
| tilt_y   |   1.82992e-05 |   0.00288553  |  -0.005    |  -0.0025   |   0       |   0.0025   |   0.005  |
| ulr      |   0.000345413 |   0.0287787   |  -0.05     |  -0.0243   |   0.0008  |   0.0257   |   0.05   |
| usr      |   0.000412737 |   0.0290316   |  -0.0499   |  -0.0261   |   0       |   0.0261   |   0.0499 |
| usr_cor  |   0.000100183 |   0.0208896   |  -0.1072   |   0        |   0       |   0        |   0.1045 |
| wdir     |  -5.4779e-05  |   0.00433478  |  -0.05     |   0        |   0       |   0        |   0.05   |
| wspd     |  -5.89143e-06 |   0.00263773  |  -0.005    |  -0.002    |   0       |   0.002    |   0.005  |
| z_boom   |  -3.66779e-06 |   0.000293067 |  -0.0005   |  -0.0003   |   0       |   0.0003   |   0.0005 |
| z_pt     | nan           | nan           | nan        | nan        | nan       | nan        | nan      |
| z_stake  | nan           | nan           | nan        | nan        | nan       | nan        | nan      |
:END:
