...
infile = "./data/L0/EGP/EGP-2016-raw.txt.hdr"
#+END_SRC
...
def flag_NAN(ds):
    ...
    flag_LUT = pd.read_csv("./flags.csv", index_col=2, usecols=(0,3,4,5))
    flag_LUT = flag_LUT[flag_LUT['#'] == '#'].drop(columns="#").dropna(how='all', axis='rows')
    
    df = pd.read_csv("./flags.csv", parse_dates=[0,1], comment="#", header=1)\
