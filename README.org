...
dtype = {}
for dt in zip(list(ds.variables)): dtype[dt] = {'dtype': 'float32'} 
outfname = outpath + '/' + outfile + ".nc"
if os.path.exists(outfname): os.remove(outfname)
ds.to_netcdf(outfname, mode='w', format='NETCDF4', compute=True) #, encoding=dtype)
#+END_SRC
...
ds = xr.open_mfdataset("./data/L1/EGP/*raw.nc", combine='by_coords', mask_and_scale=False).load()
print(ds)
#+END_SRC
