...
;AWS = ['KPC_L','KPC_U','EGP','SCO_L','SCO_U','TAS_L','TAS_A','MIT','QAS_L','QAS_M','QAS_U','NUK_L','NUK_U','NUK_K','KAN_B','KAN_L','KAN_M','KAN_U','UPE_L','UPE_U','THU_L','THU_U2'] ; run for current PROMICE stations
;AWS = ['UPE_L','UPE_U','THU_L','THU_U2'] ; run for current PROMICE stations
;AWS = ['UPE_U','THU_L','THU_U','THU_U2','CEN'] ; run for current PROMICE stations
...
upd=2; number of files to update from the bottom of the metadata file
...
;endif
;if AWS[i_AWS] eq 'QAS_M' then begin  
;bad = Where(Haws_h eq -999, nbad, COMPLEMENT=good, NCOMPLEMENT=ngood)
;IF nbad GT 0 && ngood GT 1 THEN Haws_h[bad] = INTERPOL(Haws_h[good], FLOAT(good), FLOAT(bad))
;  PRINT, 'interpolate ', n_elements(Haws_h)
;endif
