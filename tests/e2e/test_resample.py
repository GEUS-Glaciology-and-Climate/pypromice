...
from pypromice.pipeline.resample import compute_time_completeness, compute_value_completeness, apply_completeness_filters
...
def test_10min_to_hourly_with_nans():
    ...
    assert pd.infer_freq(df_res.index) in {"h", "H"}  # resample result index
    # hour bins: 00:00 and 01:00
    assert not pd.isna(filtered.loc["2025-01-01 00:00", "x"])  # complete -> kept
    assert pd.isna(filtered.loc["2025-01-01 01:00", "x"])      # incomplete -> masked


def test_hourly_to_daily_with_nans():
    ...
    assert not pd.isna(filtered.loc["2025-02-01", "x"])  # pass
    assert pd.isna(filtered.loc["2025-02-02", "x"])      # fail


def test_daily_to_monthly_ms_with_nans():
    ...
    assert not pd.isna(filtered.loc["2025-06-01", "x"])  # June kept
    assert pd.isna(filtered.loc["2025-07-01", "x"])      # July masked


def test_mixed_10min_then_hourly_to_hourly():
    ...
    assert not pd.isna(filtered.loc["2025-03-01 00:00", "x"])  # 5/6 -> pass
    assert pd.isna(filtered.loc["2025-03-01 01:00", "x"])  # 0 hourly -> failed
    assert not pd.isna(filtered.loc["2025-03-01 02:00", "x"])  # 1 hourly -> pass


def test_mixed_hourly_then_daily_to_daily():
    ...
    assert not pd.isna(filtered.loc["2025-04-01", "x"])  # 20/24 -> pass
    assert pd.isna(filtered.loc["2025-04-02", "x"])  # 6/24  -> failed
    assert not pd.isna(filtered.loc["2025-04-03", "x"])  # daily sample -> pass
