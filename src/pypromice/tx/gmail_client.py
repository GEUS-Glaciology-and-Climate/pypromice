...
class GmailClient:
    ...
    def __init__(self, server: str, port: int, account: str, password: str, chunk_size: int = 500):
        ...
        logger.info("Mailbox contains %s messages", data[0].decode())
    ...
    def fetch_mails(self, uids: list[int], chunk_size: int = 100) -> Iterator[Message]:
        """
    ...
    def _fetch_mail_chunks(self, uids: List[int]) -> Iterator[Message]:
    ...
    # UID query methods
    # --------------------------------------------------------------------------------------------
    def get_new_uids(self, last_uid:int) -> List[int]:
        """
    ...
    def get_uids_by_date(self, date: datetime | str) -> list[int]:
        """
    ...
    def search_uids_by_subject(self, subject: str) -> List[int]:
        """Return UIDs of messages with a specific subject."""
        result, data = self.mail_server.uid("search", None, f'(SUBJECT "{subject}")')
        if result != "OK":
            logger.error(f"Error searching for subject '{subject}': {data}")
            return []
        return list(map(int, data[0].split()))

    def get_uids_by_sbd_imei(self, imei: str | int) -> List[int]:
        """
    ...
    def from_config(cls, *config_files: Path) -> 'GmailClient':
        ...
        return cls(
            server=parser.get("imap", "server"),
            port=parser.getint("imap", "port"),
            account=parser.get("imap", "account"),
