...
class PayloadFormat(object):
    ...
    def readType(
        self, in_file, delimiter=","
    ):  # TODO read from .txt file as well as .csv
...
class SbdMessage(object):
    ...
    def __init__(self, content, attach, imei):
        ...
        if content:
            ...
            self.session_time = self.getKeyValue(
                content, ": ", "Time of Session (UTC)", integer=False
            )
        ...
        if attach:
            if self.checkAttachment(attach) and self.checkAttachmentName(
                attach.get_filename()
            ):
    ...
    def getPayloadFromEmail(self, attach, message_size):
        ...
        if attach != None:
            ...
            if message_size:
                assert (
                    len(sbd_payload) == message_size
                )  # TODO Provide if statement in case message_size not provided
        else:
            sbd_payload = None
        return sbd_payload
...
class EmailMessage(SbdMessage):  # TODO add initialisation from .sbd file
    ...
    def __init__(self, email_msg, sender_name):
        ...
        if self.checkEmail(email_msg):
            ...
            if self.checkSender(sender_name):
                ...
                print(
                    "Email from unrecognised sender "
    ...
    def checkEmail(self, email_msg):
        ...
        if not isinstance(email_msg, email.message.Message):
            print(
                f"Email object not valid {type(email_msg)}. "
    ...
    def getEmailBody(self):
        ...
        if self._email_msg.is_multipart():
            ...
            assert (
                not content.is_multipart()
            )  # else the decode=True on the next line makes it
            body = content.get_payload(
                decode=False
            )  # return None and break the rest of the parsing
        else:
...
class L0tx(EmailMessage, PayloadFormat):
    ...
    def __init__(
        ...
        sender_name=[
            "sbdservice",
            "ice@geus.dk",
            "emailrelay@konectgds.com",
        ],  # TODO don't hardcode sender names?
    ...
    def getFormat(self):
        ...
        if self.getFirstByte().isdigit() or (
            ...
            "watson" in self.email_data["subject"].lower()
            or "gios" in self.email_data["subject"].lower()
        ):
            return None, None, None, None, -9999, False
    ...
    def checkLength(self):
        if len(self.payload) != self.bin_len:
            print(
                f"Message malformed: expected {self.bin_len} bytes, "
    ...
    def check2BitNAN(
        self, msg, type_letter, letter_flag=["g", "n", "e"], nan_value=8191
    ):
    ...
    def isSummer(self, DataLine):
        ...
        return ("!S" in DataLine and "!M" in DataLine[-2:]) or self.bin_idx % 5 in (
            0,
            1,
        )
    ...
    def isWithInstance(self, DataLine):
        ...
        return "!I" in DataLine[-5:-3] or (
            self.bin_idx % 5 in (1, 3) and self.bin_idx != -9999
        )
    ...
    def getDataLine(self):  # TODO clean up
        ...
        if self.bin_valid and bin_msg:
                ...
                # Get byte value
                    ...
                    if type_letter.lower() == "g":
                        ...
                        entry = (
                            time.strftime(
                                "%Y-%m-%d %H:%M:%S",
                                time.gmtime(Value + self.EpochOffset),
                            )
                            + ","
                            + str(Value)
                        )
            ...
            try:
                bin_msg = "2" + bin_msg.decode(
                    "cp850"
                )  # TODO de-bug so first byte is passed (currently misses of the first "2" of the year e.g. "022" instead of "2022")
            except:
                bin_msg = ""
