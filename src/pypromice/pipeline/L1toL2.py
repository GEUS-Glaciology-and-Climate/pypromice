...
def toL2(L1: xr.Dataset,
    ...
    # Clip values to instrument range thresholds (i.e. threshold filtering)
    ...
    ds["gps_lat_qc"], ds["gps_lon_qc"], ds["gps_alt_qc"] = gps.filter(ds["gps_alt"],
                                                                      ds["gps_lat_qc"],
                                                                      ds["gps_lon_qc"],
                                                                      ds["gps_alt_qc"])
    ...
    # Filter wind direction during wind-still period (i.e. wspd==0)
    ds["wdir_qc_u"] = wind.filter_wind_direction(ds["wspd_u"],ds["wdir_qc_u"])
    if ds.attrs['number_of_booms'] == 2:
        ds["wdir_qc_l"] = wind.filter_wind_direction(ds["wspd_l"],ds["wdir_qc_l"])
    if hasattr(ds, 'wdir_i'):
        if ~ds['wdir_i'].isnull().all() and ~ds['wspd_i'].isnull().all():
            ds["wdir_qc_i"] = wind.filter_wind_direction(ds["wspd_i"],ds["wdir_qc_i"])
    ...
    if ~ds["precip_u"].isnull().all() and precip_flag:
        ds["precip_qc_u"] = precipitation.filter_lufft_errors(ds["precip_u"],
                                                              ds["t_u"],
                                                              ds["p_u"],
                                                              ds["rh_u"])
    if ds.attrs["number_of_booms"]==2:
        if ~ds["precip_l"].isnull().all() and precip_flag:
            ds["precip_qc_l"] = precipitation.filter_lufft_errors(ds["precip_l"],
                                                                  ds["t_l"],
                                                                  ds["p_l"],
                                                                  ds["rh_l"])
    ...
    ds["rh_u_wrt_ice_or_water"] = humidity.adjust(ds["rh_u"],
                                                  ds["t_u"])
    if ds.attrs["number_of_booms"]==2:
        ds["rh_l_wrt_ice_or_water"] = humidity.adjust(ds["rh_l"],
                                                      ds["t_l"])
    if hasattr(ds,"t_i"):
        if ~ds["t_i"].isnull().all():
            ds["rh_i_wrt_ice_or_water"] = humidity.adjust(ds["rh_i"],
                                                          ds["t_i"])
    ...
    # --------------------------------------------------------------------------
    # Calculating preliminary derived variables
    ...
    # Set cloud cover to nans if station not on ice
    if not is_bedrock:
        ...
        ds["cc"] = radiation.calculate_cloud_coverage(ds["dlr"],
                                                      LR_overcast,
                                                      LR_clear)
    else:
        ds["cc"] = xr.full_like(ds["dlr"], np.nan)

    # Calculate albedo from radiometer measurements
    ...
    # Calculate directional wind speed
